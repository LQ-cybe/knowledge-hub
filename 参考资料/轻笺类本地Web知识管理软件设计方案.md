...
│   resources / tags / resource\_tags /          │

│   note\_links / reports + FTS5 全文索引 +       │

│   storage\_root（文件库主目录）                  │

└─────────────────────────────────────────────┘
```

**载体选择**（待决策）：① 纯本地 Web 服务（后端启动 + 浏览器访问 [localhost](https://localhost)，最简）② Tauri/Electron 桌面壳（系统托盘 / 开机启动）。
...
├─ source\_url  来源 URL（采集的网页/微信文章、书签地址）

├─ path        相对路径（file 类型，相对 storage\_root；folder 类型为空）

├─ parent\_id   → resources.id（自引用：folder 的层级关系，file 所属文件夹）
...
resource\_tags（资源↔标签 多对多）——统一标签的底层

├─ resource\_id → resources.id（ON DELETE CASCADE）

├─ tag\_id      → tags.id

└─ UNIQUE(resource\_id, tag\_id)

note\_links（笔记↔笔记 双链/引用，存 ID 免疫改名断链）

├─ source\_id → resources.id

├─ target\_id → resources.id
...
├─ period\_start / period\_end   统计区间
...
├─ resource\_id → resources.id

├─ rel\_path    相对 storage\_root 的路径

├─ size / mime\_type / hash
```
...
* 首次启动设定 **storage\_root**（如 `D:\知识库`），此后文件管理只在这个根目录内运作

* **导入文件 = 复制进库**：把文件复制到 storage\_root（可选自动按分类建子目录），**目录外的文件一律不接受**（不引用外部路径）
...
| 维度      | 机制                        | 用途            |
| ------- | ------------------------- | ------------- |
| 文件夹（物理） | parent\_id 目录树，导入时可选目标文件夹 | 按分类逐级存放       |
| 标签（虚拟）  | resource\_tags 多对多        | 跨文件夹批量组织、图谱串联 |
...
| 📂 文件夹 | 资源化（type=folder），parent\_id 层级树，增删改 / 拖拽移动                               |
...
| 维度         | 边的来源                           | 场景                    |
| ---------- | ------------------------------ | --------------------- |
| **标签**（默认） | resource\_tags 共同标注 → 相关标签自动推导 | 虚拟组织网络                |
| **文件夹**    | resources.parent\_id 包含关系      | 目录树即图谱（文件按分类逐级存放时最直观） |
| **引用**     | note\_links                    | 笔记间双链                 |
...
&#x20; → ④ 输出干净 MD：存为 note 资源（title/source\_url/meta.images）
```
...
&#x20; - links:         { mode: keep\_marked } # keep\_marked 保留链接 | plain 转纯文本 | remove 删除

&#x20; - code\_blocks:   { detect: auto, language: auto, trim\_indent: true }  # 代码块识别+语言标注+缩进修复

&#x20; - images\_local:  { download: true, max\_width: 800 }  # 图片下载到本地并压缩

&#x20; - clean:         { remove\_ads: true, merge\_paragraphs: true }  # 去广告、合并零散段落

&#x20; - footer:        { add\_source: true }   # 文末追加来源链接/作者/日期

&#x20; - ai\_summary:    { enabled: false }     # 二期：接本地 Ollama 生成摘要
```
...
| 微信单篇               | [mp.weixin.qq.com](https://mp.weixin.qq.com) 服务端渲染，HTML 内有正文 | ★      | ✅ 首版                                 |
| 普通网页               | @mozilla/readability                                         | ★      | ✅ 首版                                 |
| 合集批量（appmsgalbum）  | 列表 JS 动态加载，需模拟滚动 / 内部接口或凭证方案                                 | ★★★    | ⏸️ 二期（借鉴 wechatDownload 凭证方案）        |
| ima 云端（ima-skills） | 腾讯 ima import\_urls API                                      | ★（零开发） | ❌ **排除**：消耗云端用量 + 数据在 ima 云端，不符合本地要求 |
...
1. 载体：纯本地 Web 服务（[localhost](https://localhost)）还是 Tauri/Electron 桌面壳？

2. 前端框架：Vue3 / React？

3. 后端语言：Node.js/ Python？（采集库 Node 生态更顺，建议 Node）

4. 文件库 storage\_root 放哪个盘 / 目录？（首次启动设定）
...
| ima（ima-skills） | import\_urls 抓取思路   | ❌ 数据在云端、消耗用量 |
...
* 轻笺：[https://boluo66.top](https://boluo66.top) ｜ [https://github.com/VeteranBoLuo/light-note](https://github.com/VeteranBoLuo/light-note) ｜ [https://boluo66.top/helpCenter](https://boluo66.top/helpCenter)
...
* wechatDownload：[https://github.com/qiye45/wechatDownload](https://github.com/qiye45/wechatDownload)
