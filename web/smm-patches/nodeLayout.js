...
function getNodeRect() {
  ...
  // 节点内追加内容（如幕布式节点描述）：撑大节点宽高，使追加内容显示在节点矩形内部而非外部
  const addedContent = this._customContentAddToNodeAdd
  const addedH = addedContent && addedContent.height ? addedContent.height : 0
  const addedW = addedContent && addedContent.width ? addedContent.width : 0
  return {
    width: Math.max(
      _width + paddingX * 2 + shapePaddingX * 2 + borderWidth,
      addedW + paddingX * 2
    ),
    height: _height + paddingY * 2 + shapePaddingY * 2 + borderWidth + addedH
  }
}
